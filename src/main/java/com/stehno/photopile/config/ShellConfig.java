...
public class ShellConfig {
    ...
    public SpringWebBootstrap springWebBootstrap(){
        ...
        bootstrap.setConfig( new Properties(){
            {
                setProperty( CRASH_VFS_REFRESH_PERIOD, refreshPeriod );
                setProperty( CRASH_SSH_PORT, port );
                setProperty( CRASH_AUTH, "simple" );
                setProperty( CRASH_AUTH_SIMPLE_USERNAME, username );
                setProperty( CRASH_AUTH_SIMPLE_PASSWORD, password );
            }
        });
        return bootstrap;
    }
